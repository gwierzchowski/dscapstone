...
Initially I will be following steps described in the course official book [@IrizarryBook], but later will try to use some results described in previous chapter and further modify model based on those early statistics.
...
- $Y_{u,i}$ - rating given by user $u$ for movie $i$. This symbol denotes random variable and is used only to describe models.
...
$$\rho_0(r) = r$$
Then we will check most obvious rounding strategy i.e. to round calculated rating prediction to nearest allowed value - i.e. the value from set ${0.5, 1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5}$. Let's call it $\rho_1$. Respective R code:
```{r, echo=TRUE}
...
### Summary
We will add two models from previous section to our results list - those which the best optimize RMSE and ACCU.
```{r}
...
## User effect
Some users may be more demanding than others, and some may just like almost every movie or be very polite while rating. Also the mental meaning of particular grade may be different (i.e. four stars for some may be high rate while for others just avarage). We will try to consider this factor now.
...
half-star's ratings. Some peple are perfoectionists and like doing the things with highest precision - those will more likely will give more precise grades (i.e. fraction grades), whole other may not care that much or just see half-strs as not aesthetic 
thing - those very rarely or just never wil give fraction rates. This seems to be strongly confirmed on graph \@ref(fig:half-stars-analysis) where users are more-less grouped into those never giving fractions rates and those using fraction rates with frequency close to 50%. So it makes sense to use different rounding function for those two groups.
So we will modify slightly our previous rounding function:
$$\rho_u(r) = 
  \begin{cases}
...
### Predictions
As user effect factor we will take average of difference of ratings from predictions of our previous model (including movie effect) taken by movies for given user: 
...
Results generally improved but are surprising. I expected that $\rho_u$ should be pretty good now, but it's RMSE is second to worst 
and accuracy is still worse that $\rho_2$. I wonder if there is still some fundamental, not accounted factor.

### Summary
Like recently we will add two models from previous section to our results list - those which the best optimize RMSE and ACCU.
```{r}
...
seems to suggest that movie year of production may have some impact on ratings given by users. So let's check if adding this factor to our model will improve it.
...
To my surprise RMSE measures degradated a bit, while Acuracy little imroved. It looks like ading movie year of production to model introduced more noise to predictions and is not very helpfull. I suppose that it was because many users have rated very few or just single movies from given epoch, and this influenced too much on prediction model. To quickly check that (without making heavy computations) I decided to try cutting-off (bring down to zero) movie epoch parameter in case user rated less than 10 movies from given epoch.
...
if (file.exists("RData/genres_users_r.RData")) {
  ...
  # Ratings are splitted by generics.
  # e.g. one record:
  #  userId movieId rating timestamp            genres              year_class
  #    1     185      5    838983525         Action|Crime|Thriller  late 90's
  # 
  # is being replaced with three records and placed into recordset rgens, e.g.:
  #  userId movieId rating timestamp            gen                 year_class
  #    1     185      5    838983525         Action                 late 90's
  #    1     185      5    838983525         Crime                  late 90's
  #    1     185      5    838983525         Thriller               late 90's
  # Note that 'genres' refers to combined generics, while 'gen' is single generic.
  ...
  # We group by user and gen and calculate average rating difference of current model and actual rating
  genres_avgs_r <- rgens %>%
    left_join(movie_avgs_r, by='movieId') %>%
    left_join(user_avgs_r, by='userId') %>%
    left_join(epoch_by_user_avgs_r, by=c('userId', 'year_class')) %>%
  ...
  # Vector of unique combination of generics assocated to all movies
  genres <-
    movies %>%
    distinct(genres) %>%
    .$genres %>%
  ...
  # Vector of unique userIDs from validation set
  userId <-
    validation %>%
    distinct(userId) %>%
    .$userId %>%
  ...
  # Function that for given genres and user will calculate the genres effect value
  #  as average of genres_avgs_r$b_ug of all generics that are included in genres
  calc_bt_status <- 0
  ...
  # Creates data frame with all userID, combined generics (genres) combinations that are present in validation set
  genres_users_r <- 
    crossing(userId, genres) %>%
    semi_join(validation)
  
  # Calculate genres effect (for validation set) by applying calc_bt() function to genres_users_r data set
  ...
  # NOTE: In those calculations we used validation data set, but not ratings from this set.
  # So this does not break rules and do not lead to overfitting.
  # Logically calculations that invollved validation set belongs to computation of predicted ratings phase (which by definition uses this set). 
}
