...
All caculation routines with full source code is contained as R code ambeded in this report sources, which are available from repository: <https://github.com/gwierzchowski/>.
...
- I noticed that the data that we worked with (_GroupLens research lab_ [@SourceData]) also contained a "movie tag" imformation. It was not extracted by corse official data prepration script, so probably cound not be used in official result, but for an extercise it would be worth to check if it could improve our predictions further.
...
Looking forward I think I will be trying to use languages like Julia or Python for my next data science related learning efforts.
